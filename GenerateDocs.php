...
// Get the template!
$replace     = "../_.js.data/GenerateDocs.html";
...
// ok, re parsing function ;)
    ...
    // Removing spaces, and retuns...
        ...
        /* IGNORE ;) */
            ...
            if ($debug) {
                echo "SOMETHING({$rettest[1][$i]}): " . $ExplodeDataNow[$x] . PHP_EOL;
            }
...
foreach ($functions as $functionName => $functionValue) {
    ...
    if ($function_before=='' && $toDo) {
        $function_before = '📝 '; // Is under construction
    }        if ($function_before=='' && $isNew) {
        $function_before = '💡 '; // Light bulb (new)
    }        if ($isInternal) {
        $function_before = '⛔️ '; // Internal use, Overwrite all others
    }
    ...
    if (!isBeta()) {
        ...
        $replaceArray['text'] .= @implode("<br />", $functionValue['text']); // PHP7
        $replaceArray['text'] .= "<br /><br /><div style='background: lightyellow;'><p>";
        $replaceArray['text'] .= @implode("<br />", $functionValue['annotation']); // PHP7
